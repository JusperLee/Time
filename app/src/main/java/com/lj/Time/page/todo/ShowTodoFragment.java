...
import com.lj.Time.presenter.todo.ShowTodoPresenterImpl;
import com.lj.Time.util.ThemeUtils;
...
public class ShowTodoFragment extends BaseFragment implements IShowTodoContract.View {
    ...
    private com.lj.Time.page.todo.ShowTodoAdapter adapter;
    ...
    protected void initView() {
        ...
        adapter = new com.lj.Time.page.todo.ShowTodoAdapter(mActivity, todoList);
        recyclerView.setLayoutManager(new LinearLayoutManager(mActivity));
        recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(position -> {
            if (todoList.get(position).getType() == 1) {
                Intent intent = new Intent(mActivity, com.lj.Time.page.todo.AddTodoActivity.class);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId() == R.id.completed){
            startActivity(new Intent(mActivity, com.lj.Time.page.todo.CompletedTodoActivity.class));
            return true;
        }else{
