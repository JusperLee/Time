...
import com.lj.Time.db.DaoSession;
...
import com.lj.Time.util.UiUtils;
import com.lj.Time.widget.CountDownTextView;
import com.lj.Time.widget.CustomAutoCompleteTextView;
import com.lj.Time.widget.NumberProgressBar;
...
public class ShowPlanAdapter extends BaseRecyclerAdapter<BaseRecyclerAdapter.ViewHolder, ShowPlanEntity> {
    ...
    public void onBindViewHolder(@NonNull BaseRecyclerAdapter.ViewHolder holder, int position) {
        ...
        if (holder instanceof RationPlanViewHolder) {
            ...
            showPlanViewHolder.imgEdit.setOnClickListener(v -> com.lj.Time.page.plan.EditPlanActivity.open(context, plan.getRationPlan().getId(), plan.getType()));
            showPlanViewHolder.tvDetail.setOnClickListener(v -> com.lj.Time.page.plan.RationPlanDetailActivity.open(context, plan.getRationPlan().getId()));
            ...
            clockPlanViewHolder.imgEdit.setOnClickListener(v -> com.lj.Time.page.plan.EditPlanActivity.open(context, plan.getClockPlan().getId(), plan.getType()));
            clockPlanViewHolder.tvDetail.setOnClickListener(v -> com.lj.Time.page.plan.ClockPlanDetailActivity.open(context, plan.getClockPlan().getId()));
        }
    }
