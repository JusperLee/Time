        ...
        return Observable.fromIterable(list)
                .map(plan -> {
                    ...
                    showPlanEntity.setPlanInfo(String.format("%s ~ %s        当前：%s%s",
                            ...
                            plan.getCurrent(),
                            plan.getUnit()));
                    showPlanEntity.setProgress(com.lj.Time.presenter.plan.PlanHelper.getProgress(plan));
                    ...
                    if (plan.getPeriodIsOpen()) {
                        ...
                        if (records != null && !records.isEmpty()) {
                            for (RationRecord record : records) {
                                if (com.lj.Time.presenter.plan.PlanHelper.isCurPeriod(plan.getPeriodPlanType(), record.getDate(), "yyyy-MM-dd HH:mm:ss")) {
